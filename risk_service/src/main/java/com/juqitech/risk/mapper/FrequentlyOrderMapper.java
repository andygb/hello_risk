 ...
 */
@Mapper
public interface FrequentlyOrderMapper {

    @Select({"<script>" +
            " SELECT t1.clientOID as clientId, COUNT(1) as orderCount FROM tm_order t1 " +
    ...
    @Select({"<script>" +
            ...
            " WHERE t1.orderStatus IN ('Paid', 'Succeeded') AND t1.isDeleted = 0 " +
            " AND t1.createTime BETWEEN #{startTime} AND #{endTime} " +
            ...
            " GROUP BY t2.buyer_id HAVING COUNT(1) >= #{riskCount} " +
            "</script>"})
    ...
    @Select({"<script>" +
            " SELECT t2.ip AS buyerIP, COUNT(t1.orderOID) AS orderCount " +
            " FROM tm_order t1 " +
            " LEFT JOIN tm_order_mapping t2 ON t1.orderOID = t2.orderOID " +
            " WHERE t1.orderStatus IN ('Paid', 'Succeeded') AND t1.isDeleted = 0 " +
            " AND t1.createTime BETWEEN #{startTime} AND #{endTime} " +
            " AND t2.ip IS NOT NULL AND t2.ip != '' " +
            " GROUP BY t2.ip " +
