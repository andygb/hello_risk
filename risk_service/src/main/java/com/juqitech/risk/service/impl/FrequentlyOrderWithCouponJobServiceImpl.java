 ...
 */
@Service
public class FrequentlyOrderWithCouponJobServiceImpl implements FrequentlyOrderWithCouponJobService {
    private final static Logger logger = LoggerFactory.getLogger(FrequentlyOrderWithCouponJobServiceImpl.class);
    @Autowired
    private FrequentlyOrderWithCouponService frequentlyOrderService;
    @Autowired
    ...
    public void queryFrequentlyOrder(RiskSensitiveDimensionEnum sensitiveDimension) {
        ...
        List<FrequentlyOrderDTO> frequentlyOrderList = null;
        ...
        if (!CollectionUtils.isEmpty(frequentlyOrderList)) {
            for (FrequentlyOrderDTO frequentlyOrder : frequentlyOrderList) {
                switch (sensitiveDimension) {
                    case SAME_CLIENT_ID:
                        riskSensitiveOrderList = frequentlyOrderService.queryFrequentlyOrderDetail(
                                frequentlyOrder.getClientId(), null, null, period.getStartTime(), period.getEndTime());
                        break;
                    case SAME_BUYER_ID:
                        riskSensitiveOrderList = frequentlyOrderService.queryFrequentlyOrderDetail(
                                null, frequentlyOrder.getBuyerId(), null, period.getStartTime(), period.getEndTime());
                        break;
                    case SAME_BUYER_IP:
                        riskSensitiveOrderList = frequentlyOrderService.queryFrequentlyOrderDetail(
                                null, null, frequentlyOrder.getBuyerIP(), period.getStartTime(), period.getEndTime());
                        break;
                    default: break;
                }
    ...
    private RiskSensitiveRecordDTO buildRiskSensitiveRecord(FrequentlyOrderDTO frequentlyOrder,
        ...
        sensitiveRecord.setSensitiveType(RiskSensitiveTypeEnum.FREQUENTLY_ORDER_WITH_COUPON);
